...
export default async function LeagueTabContent({
  id,
  tab,
  league,
  hasRule,
  ...
  const activeDayCounters = (allDayCounters ?? []).filter((c) => c.date === activeDate)
              ...
              <ul className="space-y-2 mb-5">
                {inputPlayers.map((p) => {
                  ...
                  return (
                    <li
                      key={p.player_id}
                      className="flex items-center justify-between gap-2 py-1.5 border-b border-cream last:border-0"
                    >
                      <span className="text-sm text-gray-800">{p.players?.name}</span>
                      <div className="flex items-center gap-2 flex-none">
                        {p.fromGame ? (
                          <span className="text-xs text-warm-gray bg-cream border border-warm-border px-2 py-0.5 rounded-full">
                            対局あり
                          </span>
                        ) : (
                          <>
